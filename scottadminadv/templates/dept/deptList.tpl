...
<head>
  ...
  <title>部門情報リスト | ScottAdmin 発展 Sample</title>
...
<body>
  ...
  <section>
    <p>新規登録は<a href="/oh/wp32/scottadminadv/dept/goDeptAdd.php">こちら</a>から</p>
  </section>
    ...
    <table>
      ...
      {foreach from=$deptList item='dept' name='deptListLoop'}
        <tr>
          ...
          <td>{$dept->getLoc()}</td>
          <td>
            <form action="/oh/wp32/scottadminadv/dept/prepareDeptEdit.php" method="post">
              <input type="hidden" id="editDeptDeptno" name="editDeptDeptno" value="{$dept->getDeptno()}">
              <input type="submit" value="編集">
            </form>
          </td>
          <td>
            <form action="/oh/wp32/scottadminadv/dept/connfirmDeptDelete.php" method="post">
